...
namespace qh
{
    ...
    void ProxyURLExtractor::Extract( const KeyItems& keys, const std::string& raw_url, std::string& sub_url )
    {
        ...
        while (!token.isEnd()) {
			...
			const char* curreadpos = token.getCurReadPos();
			unsigned int sizeofkey = 0;
			while (token.current() != '&' && token.current() != '=' && !token.isEnd())
			{
				token.next();
				++sizeofkey;
			}
			
			key.assign(curreadpos, sizeofkey);
